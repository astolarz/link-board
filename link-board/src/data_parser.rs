...
pub async fn get_all_trains(data_retriever: &impl DataRetriever) -> Result<Vec<Train>, Error> {
    ...
    for (route, json) in trains_json {
        ...
        #[cfg(not(feature="serde"))]
        let mut trains = parse_route_v2(&json, route)?;
...
#[cfg(not(feature="serde"))]
fn parse_route_v2(json_string: &String, route: Route) -> Result<Vec<Train>, Error> {
    // info!("{}", json_string);
    info!("json_string.len(): {}", json_string.len());
    let json: JsonValue = json_string.parse()?;
    info!("json: {}", mem::size_of_val(&json));
    let json_data = &json["data"];
    info!("json_data: {}", mem::size_of_val(&json_data));
    let references = &json_data["references"];
    info!("references: {}", mem::size_of_val(&references));
    info!("parsing stop names");
    let stops_to_names = parse_stop_names_v2(&references["stops"]);
    info!("stops_to_len: {}", stops_to_names.len());
    let trip_ref_values: Option<&Vec<JsonValue>> = references["trips"].get();
    info!("trip_refs: {}", mem::size_of_val(&trip_ref_values));
    ...
    if let Some(trip_ref_values) = trip_ref_values {
        ...
        if let Ok(trip_ids_to_dests) = parse_trips_from_refs_v2(trip_ref_values, route) {
            if let Some(trip_values) = json_data["list"].get() as Option<&Vec<JsonValue>> {
                info!("found {} tripDetails for route {:?}", trip_values.len(), route);

                for trip_value in trip_values {
                    match parse_train_from_trip_details_v2(trip_value, &trip_ids_to_dests, &stops_to_names, route) {
                        Ok(train) => trains.push(train),
                        Err(e) => {
                            if !e.is_not_in_progress_err() {
                                return Err(e);
...
#[cfg(not(feature="serde"))]
fn parse_trips_from_refs_v2(trip_values: &Vec<JsonValue>, route: Route) -> Result<HashMap<&str, Destination>, Error> {
    let mut trips_map = HashMap::new();

    for trip in trip_values {
        let id = trip["id"].get::<String>().unwrap().as_str();
        info!("directionId (JsonValue): {:?}", trip["directionId"]);
        let direction_id: Option<&f64> = trip["directionId"].get();
        if let Some(destination) = dir_id_to_destination_v2(direction_id, route) {
...
#[cfg(not(feature="serde"))]
fn parse_train_from_trip_details_v2(
        trip_detail_value: &JsonValue,
        trip_ids_to_dests: &HashMap<&str, Destination>,
        stops_to_names: &HashMap<&str, &str>,
        route: Route
    ) -> Result<Train, Error> {

    let id = trip_detail_value["tripId"].get::<String>().unwrap().as_str();
        // .ok_or_else(|| Error::trip_parse_error(TripParseErr::Id))?;

    let trip_status = &trip_detail_value["status"];

    // If scheduledDistanceAlongTrip is 0 or not present, then the trip is not currently in progress.
    if let Some(sched_dist) = trip_status["scheduledDistanceAlongTrip"].get::<f64>() {
        if *sched_dist == 0.0 {
            warn!("trip {} not in progress yet on route {:?}, scheduledDistanceAlongTrip: {}", id, route, sched_dist);
            return Err(Error::trip_parse_error(TripParseErr::NotInProgress));
        }
    ...
    let next_stop_id = trip_status["nextStop"].get::<String>().unwrap().as_str();
        // .ok_or_else(|| Error::trip_parse_error(TripParseErr::BeyondLastStop))?;

    let next_stop_name = stops_to_names.get(&next_stop_id)
        .ok_or_else(|| Error::trip_parse_error(TripParseErr::NextStop))?.to_string();

    let next_stop_time_offset = *trip_status["nextStopTimeOffset"].get::<f64>().unwrap() as i64;
        // .ok_or_else(|| Error::trip_parse_error(TripParseErr::BeyondLastStop))?;

    let closest_stop_time_offset = *trip_status["closestStopTimeOffset"].get::<f64>().unwrap() as i64;
        // .ok_or_else(|| Error::trip_parse_error(TripParseErr::ClosestStopTimeOffset))?;
...
#[cfg(not(feature="serde"))]
fn parse_stop_names_v2(json: &JsonValue) -> HashMap<&str, &str> {
    let mut stop_map = HashMap::new();

    if let Some(stops) = json.get() as Option<&Vec<JsonValue>> {
        for stop in stops {
            let stop_id= stop["id"].get::<String>().unwrap().as_str();//.as_str().unwrap_or("");
            let stop_name = stop["name"].get::<String>().unwrap().as_str();//.as_str().unwrap_or("");
...
#[cfg(not(feature="serde"))]
fn dir_id_to_destination_v2(dir_id: Option<&f64>, route: Route) -> Option<Destination> {
    // directionId can only be 0 or 1 per GTFS docs
    match dir_id {
        Some(0.0) => match route {
            Route::Line1 => Some(Destination::AngleLake),
            Route::Line2 => Some(Destination::RedmondTech),
        },
        Some(1.0) => match route {
            Route::Line1 => Some(Destination::LynnwoodCC),
            Route::Line2 => Some(Destination::SouthBellevue),
        },
        _ => {
            warn!("invalid directionId: {:?}", dir_id);
            None
        },
    }
}